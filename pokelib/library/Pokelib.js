...
define( [
...
], function ( Virtjs, Bag, GrowthRate, Hacks, Item, Move, OpponentTeam, Opponent, PlayerTeam, Player, Pokedex, Species, utilities ) {
    ...
    return Virtjs.ClassUtil.extend( [
        ...
        initialize : function ( engine ) {
            ...
            this._globalMemoryHooks = { };
            this._bankedMemoryHooks = { };

            this._engine.mmu.on( 'read', function ( e ) {
                this._checkMemoryReadHook( e );
            }.bind( this ) );
        ...
        /**
         * Register a new value which should be returned when the engine hits a specific memory address.
         *
         * Setting null disables any registered hook.
         *
         * @param {Number} bank    The ROM bank number, or null if any.
         * @param {Number} address The address.
         * @param {Number} value   The new value.
         */

        memoryHook : function ( bank, address, value ) {
            ...
            if ( bank === null ) {
                ...
                this._globalMemoryHooks[ address ] = value;

            } else {

                if ( ! this._bankedMemoryHooks[ bank ] )
                    this._bankedMemoryHooks[ bank ] = { };

                this._bankedMemoryHooks[ bank ][ address ] = value;

            }

        },

        _checkMemoryReadHook : function ( e ) {

            var bank = this._engine.environment.mbc3RomBank;
            var address = e.address;

            if ( this._bankedMemoryHooks[ bank ] && this._bankedMemoryHooks[ bank ][ address ] != null ) {
                e.override( this._bankedMemoryHooks[ bank ][ address ] );
            } else if ( this._globalMemoryHooks[ address ] != null ) {
                e.override( this._globalMemoryHooks[ address ] );
            }
